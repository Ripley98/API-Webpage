...
logging.basicConfig(filename="/home/pi/PycharmProjects/API-Webpage/venv/Build/Logs/ProcessLog", level=logging.DEBUG,
        ...
        for act in Accountid:
            ...
            if line.status_code == 404:
                ...
                if sipPort == 'SIP_4_Port_MTA':
                    origninal = r'/home/pi/PycharmProjects/API-Webpage/venv/Build/config/orig_config/VEGA_4P_AV_DISABLED_USERS_TEMPLATE_CONFIG_SEP2022vF1.txt'
                elif sipPort == 'SIP_8_Port_MTA':
                    origninal = r'/home/pi/PycharmProjects/API-Webpage/venv/Build/config/orig_config/NEW8Ptemplateconfig10122022F1.txt'
                elif sipPort == 'SIP_24_Port_MTA':
                    origninal = r'/home/pi/PycharmProjects/API-Webpage/venv/Build/config/orig_config/24PORT-VEGA3000G-config-10042022-DISABLED-USERS-AV-TEMPLATE.txt'

                newFileName = r'/tftpboot/' + mac.upper() + 'config.txt'
                newScriptFile = r'/tftpboot/' + mac.upper() + 'script.txt'

                baseScript = r'/home/pi/PycharmProjects/API-Webpage/venv/Build/config/orig_script/baseConfigScript.txt'
...
def create_device(Partitionid, Accountid, headers, mac, sipPort):
    ...
    for prt in Partitionid:
        for act in Accountid:
            ...
            logging.debug("Check if account in partition:" + str(i))
...
def adddevice():
    ...
    if apiInfo.sipPort != '' and error == []:
        ...
        if mac != None:
            while newFileName == '':
                usernames, passwords, error = create_device(apiInfo.Partitionid, apiInfo.Accountid, apiInfo.headers,
                                                            mac, apiInfo.sipPort)
                if error != []:
                    pass
                else:
                    newFileName = config_template(apiInfo.Partitionid, apiInfo.Accountid, apiInfo.headers, usernames,
                                                  passwords, apiInfo.sipPort, apiInfo.mac)
                    step = 4
                    return send_file(newFileName, as_attachment=True)
                if newFileName != '':
                    step = 4
                return render_template('adddevice.html', error=error, step=step, accountName=apiInfo.accountName,
                                       sipPort=apiInfo.sipPort, newFileName=newFileName)
...
def logs():
    ...
    return send_file(r'home/pi/PycharmProjects/API-Webpage/venv/Build/Logs/ProcessLog', as_attachment=True)
